...
export const generateImageWithText = async (
    ...
    try {
        const res = await fetch(
            `https://openai.cleveratt.com/v1/images/generations`,
            requestInit
        ).then(async (response) => {
            if (!response.ok) {
...
export const chatWithGptTurbo = async (
    ...
    try {
        const res = await fetch(
            `https://openai.cleveratt.com/v1/chat/completions`,
            requestInit
        ).then(async (response) => {
            if (!response.ok) {
                const text = await response.text();
...
export const getCurrentApiKeyBilling = async (apiKey: string) => {
    const res = await fetch(
        `https://openai.cleveratt.com/dashboard/billing/credit_grants`,
        {
            headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${apiKey}`,
